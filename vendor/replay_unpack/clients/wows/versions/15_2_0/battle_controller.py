...
class BattleController(IBattleController):
    ...
    def _getCrewInfo(self, vehicle):
        crew_params = vehicle.properties['client'].get('crewModifiersCompactParams')
        if not crew_params:
            return {
                'crew_id': None,
                'learned_skills': {}
        ...
        learned_skills_packed = crew_params.get('learnedSkills', {})
        ...
        return {
            'crew_id': crew_params.get('paramsId'),
    ...
    def onAchievementEarned(self, avatar, avatar_id, achievement_id):
        ...
        player = self._players.get_info().get(avatar_id)
        if player is None:
            # Newer replays can deliver achievement packets before the player
            # table has been populated for that avatar. Keep strict replay
            # parsing alive; later player info updates will still fill in the
            # normal roster state.
            return
        stable_avatar_id = player.get('avatarId', avatar_id)
        self._achievements.setdefault(stable_avatar_id, {}).setdefault(achievement_id, 0)
        self._achievements[stable_avatar_id][achievement_id] += 1
