    ...
    def _convert_to_dict(self, player_info, player_type):
        ...
        for key, value in player_info:
            ...
            if property_name is None:
                # New client versions sometimes add extra player fields before
                # the replay_unpack constants are updated. Keep the data under
                # a stable placeholder name instead of crashing strict replay
                # checks on an otherwise parseable replay.
                property_name = f"unknown_{key}"
            player_dict[property_name] = value
